...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  5 15:10:57 2021
...
def annual_demand_generator():
    def psycop_df(table_name):
        conn=psycopg2.connect(host = "172.18.0.2", database = "egon-data", user="egon", password="data")
        sql="SELECT * FROM {}".format(table_name)
        data = sqlio.read_sql_query(sql,conn)
        conn = None
        return data
    #function for extracting demand data from pgadmin airflow database(new one added on 3003); converts the table into a dataframe 
    def psycop_df_AF(table_name):
        conn=psycopg2.connect(host = "127.0.0.1", port='59734', database = "test-egon-again", user="egon", password="data")
        sql="SELECT * FROM {}".format(table_name)
        data = sqlio.read_sql_query(sql,conn)
        conn = None
        return data
    #the function extracts all spatial data in the form of geopandas dataframe
    def psycop_gdf(table_name):
        conn=psycopg2.connect(host = "172.18.0.2", database = "egon-data", user="egon", password="data")
        sql="SELECT * FROM {}".format(table_name)
        data = gpd.read_postgis(sql,conn)
        conn = None
        return data
    #demand, appartment per ha, building per ha, household per ha, houshold number and household prognosis 
    #from the egon-data base; Zensus population per hectare (zp_ha) is extracted as a geopandas dataframe
    demand = psycop_df_AF('demand.egon_peta_heat')
    a_pha = psycop_df('society.destatis_zensus_apartment_per_ha')
    b_pha = psycop_df('society.destatis_zensus_building_per_ha')
    h_pha = psycop_df('society.destatis_zensus_household_per_ha')
    hh_num = psycop_df('society.egon_demandregio_household')
    hh_prog = psycop_df('society.egon_household_prognosis')
    zp_pha= psycop_gdf('society.destatis_zensus_population_per_ha')
    #working only with the 2035 values
    demand= demand[demand['scenario']=='eGon2035']
    ##removing all non essential columns and repeating rows
    ...
    ##data frame with all available zensus_population_id with their corresponding grid_id
    ##no repetion of the value
    all_grid = a_pha_1.append(b_pha_1).append(h_pha_1)
    all_grid= all_grid.drop_duplicates(subset='zensus_population_id')
    ##df with only the existing demand values,corresponding zensus population id and grid id
    demand_grid =pd.merge(demand,all_grid, how='left',on='zensus_population_id')
    ...
    demand_geom = demand_geom.drop(demand_geom.columns.difference(['demand','grid_id','geom']),1)
    demand_geom['geom']=demand_geom['geom'].to_crs(epsg=4326)
    ##reading the shape file with the climate zones
            #######***add the shapefile into the database'****#######^
    ...
    schleswig_holstein=temperature_zones.loc[0:2]
    demand_zone = gpd.sjoin(demand_geom, schleswig_holstein, how ='inner' , op='intersects' )
    #final demand_zone file available as demand_zone_TC.csv
    return demand_zone
...
class IdpProfiles:
   ...
   #combination of weighted_temperature, get_normalized_bdew and temperature_inteval
    def get_temperature_interval(self, how='geometric_series'):
        ...
        return self.df
    #selecting random 24 hour idp for each day depending on the temperature class based on thier respective class pool
    def idp_selection(self):
        idp_collection_class_3 =self.idp_collection_class_3
        idp_collection_class_4 = self.idp_collection_class_4
        idp_collection_class_5 =self.idp_collection_class_5
        idp_collection_class_6 =self.idp_collection_class_6
        idp_collection_class_7 =self.idp_collection_class_7
        idp_collection_class_8 =self.idp_collection_class_8
        idp_collection_class_9 =self.idp_collection_class_9
        idp_collection_class_10 =self.idp_collection_class_10
        temperature_int = self.temperature_int[self.station_name]
        temperature_interval = temperature_int.resample('D').max()
        
        combined_idp = pd.DataFrame()
              
        for j in range(len(temperature_interval)):
            day_class = 'Class_' + str(temperature_interval[j])
            if day_class == 'Class_3':
                selected_idp = idp_collection_class_3.sample(axis='columns')
            if day_class == 'Class_4':
                selected_idp = idp_collection_class_4.sample(axis='columns')
            if day_class == 'Class_5':
                selected_idp = idp_collection_class_5.sample(axis='columns')
            if day_class == 'Class_6':
                selected_idp = idp_collection_class_6.sample(axis='columns')
            if day_class == 'Class_7':
                selected_idp = idp_collection_class_7.sample(axis='columns')
            if day_class == 'Class_8':
                selected_idp = idp_collection_class_8.sample(axis='columns')
            if day_class == 'Class_9':
                selected_idp = idp_collection_class_9.sample(axis='columns')
            if day_class == 'Class_10':
                selected_idp = idp_collection_class_10.sample(axis='columns')
            selected_idp = selected_idp.iloc[:,0]
            combined_idp = pd.concat([combined_idp, selected_idp])
        combined_idp = combined_idp.reset_index()
        combined_idp.drop(columns=['index'], inplace = True)
        combined_idp.rename(columns={0:'Demand'},inplace=True)
        combined_idp.set_index(self.df.index,inplace=True)
        return combined_idp
