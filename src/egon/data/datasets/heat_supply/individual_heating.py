...
#: Smallest heat pump capacity available on the market, in MW. The sizing rule
#: in :func:`determine_minimum_hp_capacity_per_building` is clamped to this
#: value, so no building is ever assigned a heat pump smaller than this in any
#: scenario. Taken from "Deutsche Energie-Agentur (dena, 2026):
...
class HeatPumpsCascade(Dataset):
    ...
    building that had a heat pump in the previous link keeps at least its
    inherited capacity -- raised where its own peak heat demand has grown
    since -- and only the remaining capacity of the MV grid district is
    distributed
...
def determine_minimum_hp_capacity_per_building(
    ...
    Notes
    -----
    The result is clamped to :data:`MIN_HP_CAPACITY`, the smallest heat pump
    available on the market. Buildings whose heat demand would call for a
    smaller unit are assigned that minimum size rather than being excluded
    from heat pump supply: the clamp is a lower bound on the size, not a
    filter on the buildings.

    Clamping here rather than at the call site makes the bound structural --
    every caller, and therefore every scenario, is covered. Combined with the
    inheritance floor in :func:`determine_hp_cap_peak_load_mvgd_ts_status_quo`
    and friends, which take the maximum of the inherited and the currently
    required capacity, this guarantees inductively that no heat pump anywhere
    on the floor chain is smaller than :data:`MIN_HP_CAPACITY`.

    """
    return (peak_heat_demand * flexibility_factor / cop).clip(
        lower=MIN_HP_CAPACITY
    )
