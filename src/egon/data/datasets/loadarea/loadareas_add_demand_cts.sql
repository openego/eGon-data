/*
Add consumption and peak loads to load areas for sector: CTS
...
-- Add CTS consumption
UPDATE demand.egon_loadarea AS t1
    SET sector_consumption_cts_2037_2025 = t2.demand
    FROM (
        SELECT  a.id AS id,
                SUM(b.demand)::float AS demand
        FROM    demand.egon_loadarea AS a,
...
-- Add CTS peak load
UPDATE demand.egon_loadarea AS t1
    SET sector_peakload_cts_2037_2025 = t2.peak_load_in_mw
    FROM (
        SELECT  a.id AS id,
                SUM(b.peak_load_in_w)/1000000::float AS peak_load_in_mw
        FROM    demand.egon_loadarea AS a,
...
UPDATE demand.egon_loadarea AS t1
    SET
        sector_consumption_cts = sector_consumption_cts_2035 * t2.scaling_factor,
        sector_peakload_cts = sector_peakload_cts_2035 * t2.scaling_factor
    FROM (
        SELECT
            la.id,
            dr.scaling_factor
