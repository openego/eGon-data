...
def sanitycheck_electricity():
    ...
    b1 = sum_installed_gen_cap_DE["Error"].values[3]
    b = round(b1, 2)
    
    c1 = sum_installed_gen_cap_DE["Error"].values[4]
    c = round(c1, 2)
    
    d1 = sum_installed_gen_cap_DE["Error"].values[0]
    d = round(d1, 2)
    
    f1 = sum_installed_gen_cap_DE["Error"].values[1]
    f = round(f1, 2)
