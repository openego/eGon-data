...
def select_chp_from_nep(sources):
    ...
    chp_NEP = chp_NEP.append(
        chp_NEP_data[chp_NEP_data.name.isnull()].loc[:, [
            ...
            'c2035_capacity',  'c2035_chp', 'city', 'federal_state']])
    # Insert rows from list with a name
    chp_NEP = chp_NEP.append(
        chp_NEP_data.groupby(
            ['carrier', 'name', 'postcode', 'c2035_chp', 'city', 'federal_state']
            )['capacity','c2035_capacity', 'city', 'federal_state'].sum().reset_index()).reset_index()
...
def insert_large_chp(sources, target, EgonChp):
    ...
    MaStR_konv = MaStR_konv.groupby(
        ['plz', 'Laengengrad', 'Breitengrad',
         'carrier', 'city', 'federal_state']
    ...
    chp_NEP_matched, MaStR_konv, chp_NEP = match_nep_chp(
        chp_NEP, MaStR_konv, chp_NEP_matched,
        buffer_capacity=0.1)
    ...
    chp_NEP = chp_NEP.groupby(
        ['postcode', 'carrier', 'city', 'c2035_chp', 'federal_state']
        )[['capacity', 'c2035_capacity']].sum().reset_index()
    ...
    insert_chp = chp_NEP_matched.groupby(
