...
def profile_generator(aggregation_level):
    ...
    if aggregation_level == 'district':
        ...
        heat_profile_idp = pd.merge(heat_profile_idp,mv_grid_ind['bus_id'], left_on =selected_profiles.index,
...
def residential_demand_scale(aggregation_level):
    ...
    if aggregation_level == 'district':
        ...
        district_grid =pd.merge(mv_grid_ind[['bus_id','zensus_population_id']],
                      annual_demand[['zensus_population_id','Station','demand']],
                      on='zensus_population_id', how ='inner')
        ...
        for j in range(len(heat_profile_idp.columns)):
            ...
            station =  district_grid[ district_grid.index == bus_id]['Station'][bus_id]
            if type(station)!=str:
                station = station.reset_index()
                multiple_stations = pd.DataFrame()
                for i in station.index:
                ...
                demand_curves_mv[bus_id] = current_district.multiply(h[station],axis=0)
        ...
        heat_demand_profile_mv.rename(columns={'key_0': 'bus_id'},inplace=True)
...
def cts_demand_per_aggregation_level(aggregation_level):
    ...
    if aggregation_level == 'district':
        ...
        CTS_per_grid = pd.merge(CTS_per_zensus,mv_grid_ind[['bus_id','zensus_population_id']],
                                     on='zensus_population_id',
                                     how='inner') 
...
def CTS_demand_scale(aggregation_level):
    ...
    if aggregation_level =='district':
        ...
        CTS_demands_grid = pd.merge(demand,mv_grid_ind[['bus_id','zensus_population_id']],
                                     on='zensus_population_id',
                                     how='inner') 
